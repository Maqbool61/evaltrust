...
### Fixed
- **Fixed file decoding for non-UTF-8 default locales.** The CLI now explicitly uses UTF-8 when reading   benchmark and reference files, preventing `UnicodeDecodeError` on systems (like Windows) where the default locale encoding is not UTF-8.
